...
const checkEmoji = (header) => {
  ...
  const code = header.split(':')[1].trim()
...
export default {
  ...
  plugins: [
    {
      rules: {
        'type-enum-rule': ({ header }) => [checkType(header), `❌  submit type error,optional values are: ${enumType.join(',')}`],
        'subject-enum-rule': ({ header }) => [checkSubject(header), '❌  need to fill in the commit subject'],
        'subject-emoji-rule': ({ header }) => [checkEmoji(header), '❌  using emoji!!!!!!!!!!!!!!'],
        'error-tip': ({ header }) => [checkType(header) && checkSubject(header) && checkEmoji(header), '💡 example: "feat: 😁add xxx"'],
      },
    },
  ],
}
